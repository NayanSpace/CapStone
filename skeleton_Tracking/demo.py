...
while True:
    ...
    #NEW Publish joint angles to ROS2 topic
    joint_angles = renderer.get_joint_angles()
    joint_angles_publisher.publish_joint_angles(joint_angles)
    
    key = renderer.waitKey(delay=1)
    if key == 27 or key == ord('q'):
        break
joint_angles_publisher.shutdown()     
renderer.exit()
tracker.exit()
